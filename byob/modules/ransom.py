...
def decrypt_file(filename, key):
    ...
    try:
        if os.path.isfile(filename):
            ...
            with open(filename, 'wb') as fd:
                fd.write(plaintext)
            util.debug('{} decrypted'.format(filename))
            return True
        else:
            _debugger.debug("File '{}' not found".format(filename))
    except Exception as e:
